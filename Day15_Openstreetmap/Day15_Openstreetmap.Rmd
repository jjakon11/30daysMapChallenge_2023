...
river <- opq(bbox = Cod) %>%
  ...
  osmdata_sf()
buildings <- opq(bbox = Cod) %>%
  add_osm_feature(key = "building") %>%
  osmdata_sf()

leisure <- opq(bbox = Cod) %>%
  add_osm_feature(key = 'leisure', value = c('garden','park')) %>%
  osmdata_sf()

water<- opq(bbox = Cod) %>%
  add_osm_feature(key = 'natural', value = c('water', 'pond')) %>%
  osmdata_sf()
grass<- opq(bbox = Cod) %>%
...
ggexport(Chia, filename = "Chia.png", width = 14600, height=12000, res=900)
